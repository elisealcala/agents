...
describe("M3 offline workflow", () => {
  it("stores documents, learns a correction, suggests without moving, and answers with citations", async () => {
      ...
      await Promise.all(
        destinations.map(
          async (destinationPath) =>
            [destinationPath, await readFile(destinationPath)] as const,
        ),
      ),
    );
    ...
    expect(JSON.parse(await readFile(outputPath, "utf8"))).toEqual(clustering);
    ...
    expect(
      answer.sources.every(({ path: sourcePath }) =>
        sourcePath.includes("q3-cache"),
      ),
    ).toBe(true);
