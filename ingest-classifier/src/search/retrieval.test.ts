...
function provider(vector: number[]) {
  const embed = vi.fn<EmbeddingProvider["embed"]>(async () => vector);
  ...
  it("embeds only the query and ranks reusable stored vectors by cosine score", async () => {
    ...
    documents.upsert(
      input(
        ...
        "related.md",
        [0.8, 0.6],
        "Related cache invalidation guidance.",
      ),
    );
    documents.upsert(
    ...
    documents.upsert(
      input(
        ...
        "missing.md",
        null,
        "A missing vector must never be embedded at query time.",
      ),
    );
...
describe("answerQuestion", () => {
  ...
  it("admits when no stored document is sufficiently relevant", async () => {
    const documents = await openDocuments();
    documents.upsert(
      input(1, "architecture.md", [1, 0], "Architecture notes."),
    );
  ...
  it("grounds the model prompt and attaches citations only from ranked records", async () => {
    ...
    documents.upsert(
      input(
        ...
        "cache.md",
        [1, 0],
        "Measure cache hit rate and use explicit TTL values.",
      ),
    );
    documents.upsert(
      input(
        ...
        "rollout.md",
        [0.8, 0.6],
        "Rehearse invalidation before rollout.",
      ),
    );
    documents.upsert(
    ...
    expect(
      result.sources.some(
        ({ path: sourcePath }) => sourcePath === "/invented.md",
      ),
    ).toBe(false);
