...
describe("moveWithoutOverwrite", () => {
  it("moves the exact bytes and returns their SHA-256", async () => {
    ...
    await expect(
      moveWithoutOverwrite(source, destinationFolder, sha256),
    ).resolves.toEqual({
      sourcePath: source,
    ...
    expect(
      (await readFile(path.join(destinationFolder, "note.md"))).equals(bytes),
    ).toBe(true);
  });
  ...
  it("preserves collisions and chooses the next available numeric suffix", async () => {
    ...
    await writeFile(
      path.join(destinationFolder, "note-2.md"),
      "second",
      "utf8",
    );
    ...
    expect(result.destinationPath).toBe(
      path.join(destinationFolder, "note-3.md"),
    );
    await expect(
      readFile(path.join(destinationFolder, "note.md"), "utf8"),
    ).resolves.toBe("first");
    await expect(
  ...
  it("leaves the source intact when the destination cannot be created", async () => {
    ...
    await expect(
      moveWithoutOverwrite(source, missingDestination),
