...
function provider(id = "fixture-v1", dimensions = 3) {
...
describe("CategoryStore embeddings", () => {
  it("embeds every missing category and reuses persisted vectors from the same provider", async () => {
    ...
    for (const category of store.list()) {
      ...
      expect(fixtureProvider.embed).toHaveBeenCalledWith(
    ...
    expect(
      restarted
        .list()
        .every(({ embeddingProvider }) => embeddingProvider === "fixture-v1"),
    ).toBe(true);
  });
