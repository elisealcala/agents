...
export class CorrectionStore {
  ...
  record(input: {
    ...
    if (!input.originalPath.trim())
      throw new Error("correction path is required");
    if (!input.wrongCategory.trim())
      throw new Error("wrong category is required");
    if (!input.correctCategory.trim())
